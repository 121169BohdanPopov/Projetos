...
Image ImageCopy(const Image img) {
  ...
  //Make a nested loop to go to all image pixels
  for(uint32 i = 0; i<img->height;i++){
...
int ImageIsEqual(const Image img1, const Image img2) {
  ...
  // TO BE COMPLETED
  // ...
  //Compare pixel to pixel RGB 
  //TWO images are equal if have the same size
  
  if(img1->width != img2->width || img1->height != img2->height){
    return 1;//Images are not equal in size
  }

  //If they have the same size, check the rgb color in each image
  for(uint32 i = 0; i<img1->height; i++){
    for(uint32 j = 0; j<img1->width; j++){
      //Get the current pixel ppointer
      uint16 index = img1->image[i][j];
      //Get the LUT color of that pixel in each image
      rgb_t color1 = img1->LUT[index];
      rgb_t color2 = img2->LUT[index];
      if(color1 != color2){ //Compare the RGB
        return 1;//Not the same RGB Code
      }
    }
  }


  return 0;//All the RGB Pixels are the same in each position
}

...
int ImageRegionFillingRecursive(Image img, int u, int v, uint16 label) {
  ...
  // check if the old one label is the same as new label which is indicated in the function
  if(old_label == label) //  If the pixel already has the new label, nothing to fill
    return 0;
  ...
  int count = 1;  //counting unit of labeled pixels

  // fill recursively 
  if(ImageIsValidPixel(img, u + 1, v) && img->image[v][u+1] == old_label){ //Check if the neighbor is inside the image
    count+= ImageRegionFillingRecursive(img, u+1, v, label);// recur to fill the neighbor 
  }
  if(ImageIsValidPixel(img, u -1, v)&& img->image[v][u-1] == old_label){
    count+= ImageRegionFillingRecursive(img, u-1, v, label);
  }
  if(ImageIsValidPixel(img, u, v+1)&& img->image[v + 1][u] == old_label){
  ...
  return count;     // total filled pixels (labeled pixels)
}
...
int ImageRegionFillingWithSTACK(Image img, int u, int v, uint16 label) {
    ...
    uint16 old_label = img->image[v][u]; // the old label in certain place given by input u and v 
    ...
    Stack* stack = StackCreate(img->width*img->height); //creating the stack with max size width * height
    
    PixelCoords p = {u, v}; //pixel position

    img-> image[v][u] = label; // changing the label in initial point

    StackPush(stack, p); // add pixel to the stack

    while(!StackIsEmpty(stack) ) { // cicle execution when stack is not empty to check all positions
        PixelCoords cur = StackPop(stack);//get the pixel to process next
        int x = cur.u; //current position of x
        int y = cur.v; // current position of y

        PixelCoords neighbors[4] = {                // all possible ways to check
            {x+1, y}, {x-1, y}, {x, y+1}, {x, y-1}  // the neighbors by floodfill method
        };

        for(int i=0; i<4; i++) {  // cycle to see all ways
            int nx = neighbors[i].u;  //current x by all possible ways 
            int ny = neighbors[i].v;  //current y by all possible ways

            if(!ImageIsValidPixel(img, nx, ny)) continue; // Skip if the neighbor is outside the image

            if(img->image[ny][nx] ==old_label) {  
                img->image[ny][nx] =label;   // changing the label of neighbor pixel
                StackPush(stack, neighbors[i] ); // adding the neighbors to stack
                count++; //  increases the counter of  new labeled pixel
            }
        }
    }

    StackDestroy(&stack); // free stack memory
    return count; // return the count of all labeled pixels
}
...
int ImageRegionFillingWithQUEUE(Image img, int u, int v, uint16 label) {
    ...
    Queue* queue = QueueCreate(img->width*img->height); //creating the queue with max size width * height
    
    PixelCoords p = {u, v};//pixel position

    // changing the label in initial point
    img->image[v][u] = label;
    
    QueueEnqueue(queue, p); //add pixel to queue

    while(!QueueIsEmpty(queue)) { // cicle execution when stack is not empty to check all positions
        PixelCoords cur = QueueDequeue(queue);//get the pixel to process next
        int x = cur.u; //current position of x
        int y = cur.v; // current position of y
        ...
        for(int i=0; i<4; i++) {  // cycle to see all ways
            ...
            if(img->image[ny][nx]== old_label) {  
                img-> image[ny][nx ] = label;   // changing the label of neighbor pixel
                QueueEnqueue( queue, neighbors[i] ); //add to the queue neighbors
                count++; //increases the counter of  new labeled pixel
            }
        }
    }

    QueueDestroy(&queue );// free queue memory
    return count; // return the count of all labeled pixels
}
...
int ImageSegmentation(Image img, FillingFunction fillFunct) {
  ...
  // TO BE COMPLETED
  int region_count = 0;   // counter of filled regions
  rgb_t last_color = 0;   //the starting color
  for (uint32 i = 0; i < img->height; i++) {  //cicle for height
    for (uint32 j = 0; j < img->width; j++) { //cicle for width
      if(img->image[i][j] == 0){  // check if is white
        rgb_t new_color = GenerateNextColor(last_color);  // new color generation
        last_color = new_color; //starting color is the new generated color

        uint16 new_label = img->num_colors; //adding a new label by num_colors of image
        if(new_label<FIXED_LUT_SIZE){ //check if the new label not exceeds LUT_SIZE (1000)
          img->LUT[new_label] = new_color; //adding to LUT the new color
          img->num_colors++;  // increases the number of colors of image
        }

        fillFunct(img, j, i, new_label);//filling the new color to the area
        region_count++;//increases the counter of filled regions
      }
    }
  }
  return region_count;// retrurn the number of all filled regions
}
